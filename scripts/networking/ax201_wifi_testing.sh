...
IFS=$'\n\t'
if ((BASH_VERSION[0] < 5)); then
    printf 'Error: This script requires ZSH 5.0 or newer.\n' >&2
    exit 1
fi
...
check_deps() {
    ...
    if ((${#missing_pkgs[@]} > 0)); then
        ...
        case "$choice" in
        1)
            pacman -S --noconfirm --needed "${missing_pkgs[@]}" || die "Failed to install dependencies."
            ;;
        2)
            pacman -Syu --noconfirm --needed "${missing_pkgs[@]}" || die "Failed to install dependencies."
            ;;
        *)
...
validate_path() {
    ...
    local char i
    for ((i = 0; i < ${#dangerous_chars} - 1; i++)); do
