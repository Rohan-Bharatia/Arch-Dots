...
detect_battery() {
    ...
    local dev=$(upower -e 2>/dev/null | grep -m1 -iE 'BAT|battery')
    if [[ -z "$dev" ]]; then
        return 1
    fi
    printf '%s' "$dev"
...
read_battery() {
    local dev="$1"
    local info=$(upower -i "$dev" 2>/dev/null) || return 1
    [[ -z "$info" ]] && return 1
    local state perc
    ...
    case "${state,,}" in
        discharging|"not charging"|not-charging)
            state="Discharging"
            ;;
        charging|"pending charge"|pending-charge)
            state="Charging"
            ;;
