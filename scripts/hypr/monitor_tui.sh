...
[[ -f "${CONFIG_DIR}/conf/monitors.conf" ]] && CONFIG_FILE="${CONFIG_DIR}/conf/monitors.conf"
...
get_field() {
    ...
    get_active_json | jq -r --arg n "$name" --arg f "$field" '.[] | select(.name==$n) | .[$f] // empty'
}
...
get_position() {
    ...
    case $REPLY in
        1)
            printf '%dx%d' "$((ax + law))" "$ay"
            ;;
        2)
            printf '%dx%d' "$((ax - law))" "$ay"
            ;;
        3)
            printf '%dx%d' "$ax" "$((ay - lah))"
            ;;
        4)
            printf '%dx%d' "$ax" "$((ay + lah))"
            ;;
...
show_status() {
    ...
    hyprctl monitors | while IFS= read -r line; do
        case "$line" in
            ...
            *vrr:*)
                ...
                ;;
            *scale:*|*transform:*|*availableModes*)
                printf '%s\n' "$line"
                ;;
            *[0-9]x[0-9]*@*)
                printf '%s%s%s\n' "$C_GRN" "$line" "$C_R"
    ...
    case $vrr in
        0)
            vrr_str="${C_DIM}0 (off)${C_R}"
            ;;
        1)
            vrr_str="${C_GRN}1 (on)${C_R}"
            ;;
        2)
            vrr_str="${C_CYN}2 (fullscreen)${C_R}"
            ;;
        3)
            vrr_str="${C_CYN}3 (fullscreen video/game)${C_R}"
            ;;
        *)
            vrr_str="$vrr"
            ;;
