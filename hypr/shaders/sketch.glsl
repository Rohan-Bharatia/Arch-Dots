...
const float edge_softness    = 0.08;
const float line_thickness   = 1.0;
const float line_darkness    = 0.05;
const float paper_brightness = 0.98;
const float paper_grain      = 0.03;
const float noise_reduction  = 0.5;
...
float sample_lum(vec2 uv) {
    vec2 clean_uv = clamp(uv, 0.0, 1.0);
    return luminance(texture(tex, clean_uv).rgb);
}
...
void main() {
    ...
    // Calculate local variance -> reduce noise
    ...
    float edge               = smoothstep(adaptive_threshold - edge_softness,
    ...
    fragColor = vec4(vec3(mix(paper, line_darkness, edge)), 1.0);
}
